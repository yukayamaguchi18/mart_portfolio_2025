...
select
    ...
    oh.status,
    case
        when bool_or(p.product_category = 'Regular') then 'include_main'
        else 'trial_only'
    end as order_category1,
    case
        when bool_or(od.order_type = 'Subscription') then 'include_subscription'
        else 'spot_only'
...
group by
    ...
    c.member_type, oh.status, oh.total_amount;
